...
void go(int st = 0) {
    ...
    if (already == N) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            mp[i] = perm[i];
            // if (all[perm[indexes[-all[i]]]] != -all[perm[i]]) {
            //     unexpected_with_neg = true;
            // }
        }
...
bool find_bad_aut_with_stable_akb() {
    ...
    vec<int> p(akb.size());
    for (int i = 0; i < akb.size(); i++) p[i] = i;
    vec<vec<int>> bad_p;
    do {
        map<int, int> A;
        for (int i = 0; i < akb.size(); i++)
            A[i] = p[i];
        if (!is_standard_automorphism(akb, A)) {
            bad_p.push_back(p);
        }
    ...
    for (auto bp : bad_p) {
        for (int i = 0; i < akb.size(); i++)
            add(indexes[akb[i]], indexes[akb[bp[i]]]);
        go();
        if (cnt) {
            if (!check()) {
                cout << "ERROR" << endl;    
        ...
        cnt = 0;
        for (int i = 0; i < akb.size(); i++)
            pop(indexes[akb[i]], indexes[akb[bp[i]]]);
    }
...
int main() {
    ...
    add(indexes[-E], indexes[-E]); 
    add(indexes[mat({{0, 1}, {1, 0}})], indexes[mat({{0, 1}, {1, 0}})]);
    add(indexes[mat({{0, -1}, {-1, 0}})], indexes[mat({{0, -1}, {-1, 0}})]);
